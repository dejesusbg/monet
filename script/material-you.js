function addRipple(...e) {
  e.forEach((e) =>
    document.querySelectorAll(e).forEach((e) => e.classList.add("ripple"))
  );
}
...
const closeDialog = () =>
  document.querySelectorAll(".md3-dialog").forEach((e) => e.close());

function backdropDialog(e, closeEv) {
  if (e.target.tagName !== "DIALOG") return;

  const rect = e.target.getBoundingClientRect();
  const hasClickInDialog =
    rect.top <= e.clientY &&
    e.clientY <= rect.top + rect.height &&
    rect.left <= e.clientY &&
    e.clientX <= rect.left + rect.width;

  if (!hasClickInDialog) closeEv();
}

function customCloseDialog(dialog, prevent = false, closeEv = null) {
  closeEv ??= () => {
    return prevent ? dialog.showModal() : dialog.close();
  };
  ...
  document.addEventListener("click", (e) => backdropDialog(e, closeEv));
}

document.addEventListener("click", (e) => {
  backdropDialog(e, () => e.target.close());
});
...
function openMenu(trigger, e, type) {
  ...
  document
    .querySelectorAll(".md3-menu[open='true']")
    .forEach((m) => m.setAttribute("open", false));
  ...
  if (
    (bodyRect.width < 512 || bodyRect.height < 512) &&
    menu.classList.contains("md3-menu--sheet")
  ) {
  ...
  if (trigger.tagName === "BUTTON") {
    if (mid.top) {
      ...
      (y = rect.top - menuRect.height - 4), (origin = "bottom");
    }

    if (mid.left) {
      x = rect.left;
    } else if (mid.right) {
      x = rect.left - menuRect.width + rect.width;
    }
  } else {
    let originX, originY;
    if (mid.top) {
      ...
      (y -= menuRect.height - 2), (originY = "100%");
    }

    if (mid.right) {
      (x -= menuRect.width - 2), (originX = "100%");
    } else if (mid.left) {
      originX = "0";
    }
...
const closeMenu = () =>
  document
    .querySelectorAll(".md3-menu, .md3-menu-scrim")
    .forEach((e) => e.setAttribute("open", false));
const closeMenuOnOutsideClick = (e, type) => {
  if (!(e.target.offsetParent ?? e.target).getAttribute(type)) closeMenu();
};

document.querySelectorAll("[click]").forEach((e) =>
  e.addEventListener("click", function (e) {
    openMenu(this, e, "click");
  })
);
document.querySelectorAll("[contextmenu]").forEach((e) =>
  e.addEventListener("contextmenu", function (e) {
    openMenu(this, e, "contextmenu");
  })
);

document.addEventListener("click", function (e) {
  closeMenuOnOutsideClick(e, "click");
});
document.addEventListener("contextmenu", function (e) {
  closeMenuOnOutsideClick(e, "contextmenu");
});
...
function topbarScroll() {
  ...
  if (!topbar) return;
  if (isTop) {
    topbar.classList.remove("md3-topbar--scrolled");
  } else {
    topbar.classList.add("md3-topbar--scrolled");
  }
}
